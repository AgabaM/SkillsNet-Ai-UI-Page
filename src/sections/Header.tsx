...
export const loginItems = [
  ...
  name: string;
  href: string;
  buttonVariant: ButtonProps['variants'];
}[];
        ...
        <div className='h-full hidden lg:block'>
          <nav className=' h-full'>
            {navItems.map(({name, href}) =>(
                <a key={href} href={href} className='h-full px-10 relative font-bold text-xs tracking-widest text-gray-400 
                uppercase inline-flex items-center before:content-[""] before-absolute before:botton-0 before:h-2 before:w-px before:bg-gray-200/20 before:left-0
                last:after:content-[""] last:after-absolute last:after:botton-0 last:after:h-2 last:after:w-px last:after:bg-gray-200/20 last:after:right-0'
                onClick={(e) => {
                  e.preventDefault();
        ...
        <div className='hidden lg:flex gap-4'>
          {loginItems.map(({ buttonVariant, name, href }) => (
            <a href={href} key={name}>
              <Button  variants={buttonVariant}>{name}</Button>
            </a>
          ))}
        </div>
          ...
          <nav className='flex flex-col items-center gap-4 py-8 h-full justify-center'>
            ...
            {loginItems.map(({buttonVariant, name, href }) =>(
              <a href={href} key={name} className='w-full max-w-xs'>
                <Button block variants={buttonVariant}>
                  {name}
                  </Button>
                </a>
            ))}
